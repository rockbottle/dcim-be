...
def create_dc_usage(db: Session, request: DcBase, current_user: dict):
    ...
    if existing_purchase:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Usage record already exists for the company '{company_id}'."
        )
...
def update_dc_usage(db: Session, request: DcUpdate, current_user: dict):
    ...
    if not usage:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No usage record found for your company.")
    ...
    db.commit()
    db.refresh(usage)
    return {"id": usage.id, "dcpower": usage.dcpower, "uspace": usage.uspace, "nport": usage.nport, "sport": usage.sport}
